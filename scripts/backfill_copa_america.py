#!/usr/bin/env python3
"""
scripts/backfill_copa_america.py -- Copa America 2024 highlights backfill.

Manual trigger only (workflow_dispatch via backfill-copa-america.yml).
Never wired into the scheduled 4-hour fetch Action.

Fetches all 32 Copa America 2024 fixtures from API-Sports (free tier), then
...
can assess Copa America highlight coverage on YouTube.
...
#
# Copa America backfill is a manual one-shot run against a completed tournament.
    ...
    def increment(self, cap: int) -> None:
        ...
        if self.units_used >= cap:
            raise QuotaCapReached(
                f"YouTube cap reached during Copa America backfill: "
...
def _finish(
    ...
    log.info("=" * 60)
    log.info("Copa America 2024 backfill complete")
    ...
    _git_commit(
        commit_files,
        "chore: backfill Copa America 2024 highlights [skip ci]",
    )
