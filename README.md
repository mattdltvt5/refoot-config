...
renamed or removed; the client still renders from name/crest). This is the prerequisite for
**favorite-team filtering** on the home screen: the FD id is the stable cross-competition join key
(same id for a club in its domestic league and in UCL/UEL — e.g. Real Madrid `86`, verified equal in
`home-index` and `fixtures/laliga/2026.json`), so the client can filter cached home fixtures by a
favorited team id with no fan-out or live calls. Emitted `null`-safe: a fixture missing a team id (or
a partially-populated side) yields `"id": null` and never crashes the build. Change is in
`build_home_index.py::_team()`; the in-range `home-index/*` files were regenerated by running the
builder (per-season fixture files, which already carried the id, are untouched). Scope: home-index is
club/football-data; the FD↔API-Sports id-space mismatch (national teams / Copa) is out of scope here.
See `docs/favorite-teams-feasibility.md`.
